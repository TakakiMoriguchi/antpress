defmodule AntPressWeb.UserLive.ConfirmationTest do
  ...
  describe "Confirm user" do
    ...
    test "confirms the given token once", %{conn: conn, unconfirmed_user: user} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/client/log-in/#{token}")
      ...
      {:ok, _lv, html} =
        live(conn, ~p"/client/log-in/#{token}")
    ...
    test "raises error for invalid token", %{conn: conn} do
      {:ok, _lv, html} =
        live(conn, ~p"/client/log-in/invalid-token")
        |> follow_redirect(conn, ~p"/client/log-in")
