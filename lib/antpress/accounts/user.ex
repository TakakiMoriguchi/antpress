defmodule AntPress.Accounts.User do
  use Ecto.Schema
  ...
  schema "users" do
    ...
    # オーナーは developer が発行、スタッフはオーナーが発行
  ...
  @doc """
  ユーザーを新規作成するための changeset。

  **オーナーは developer が発行、スタッフはオーナーが発行する。**
  セルフサインアップは行わない（→ `docs/DECISIONS.md` 1.3）。

  ⚠️ `client_id` は `cast` しない。呼び出し側がスコープから `put_change` する。
  attrs から受け取ると、他のクライアント配下にユーザーを作れてしまう。
  """
  def create_changeset(user, attrs, opts \\ []) do
    ...
    |> validate_email(opts)
    |> check_constraint(:role, name: :users_role_valid)
  end

  @doc """
  表示名の更新。`role` と `client_id` は含めない（誤って権限や所属を変えないため）。
  """
  ...
  @doc """
  オーナーかどうか。スタッフはスタッフを発行できず、フォーム設定も変更できない
  （→ `docs/SCREENS.md` 6 章）。
  """
  ...
  """
  def valid_password?(%AntPress.Accounts.User{hashed_password: hashed_password}, password)
