defmodule AntPressWeb.DeveloperLive.Settings do
  use AntPressWeb, :live_view

  on_mount {AntPressWeb.DeveloperAuth, :require_sudo_mode}

  alias AntPress.Platform

  @impl true
  def render(assigns) do
    ~H"""
    <Layouts.app flash={@flash} current_developer={@current_developer}>
      <div class="text-center">
        <.header>
          アカウント設定
          <:subtitle>メールアドレスとパスワードを変更できます</:subtitle>
        </.header>
      </div>

      <div class="flex items-center justify-between gap-4 rounded-lg border border-base-300 p-4">
        <div>
          <p class="font-semibold">表示テーマ</p>
          <p class="text-sm opacity-60">システム設定に従う / ライト / ダーク</p>
        </div>
        <AntPressWeb.Layouts.theme_toggle />
      </div>

      <.form
        for={@profile_form}
        id="profile_form"
        phx-submit="update_profile"
        phx-change="validate_profile"
      >
        <.input field={@profile_form[:name]} type="text" label="屋号・氏名" required />
        <.button variant="primary" phx-disable-with="保存中...">保存</.button>
      </.form>

      <.form for={@email_form} id="email_form" phx-submit="update_email" phx-change="validate_email">
        <.input
          field={@email_form[:email]}
          type="email"
          label="Email"
          autocomplete="username"
          spellcheck="false"
          required
        />
        <.button variant="primary" phx-disable-with="Changing...">Change Email</.button>
      </.form>

      <div class="divider" />

      <.form
        for={@password_form}
        id="password_form"
        action={~p"/developers/update-password"}
        method="post"
        phx-change="validate_password"
        phx-submit="update_password"
        phx-trigger-action={@trigger_submit}
      >
        <input
          name={@password_form[:email].name}
          type="hidden"
          id="hidden_developer_email"
          spellcheck="false"
          value={@current_email}
        />
        <.input
          field={@password_form[:password]}
          type="password"
          label="New password"
          autocomplete="new-password"
          spellcheck="false"
          required
        />
        <.input
          field={@password_form[:password_confirmation]}
          type="password"
          label="Confirm new password"
          autocomplete="new-password"
          spellcheck="false"
        />
        <.button variant="primary" phx-disable-with="Saving...">
          Save Password
        </.button>
      </.form>
    </Layouts.app>
    """
  end

  @impl true
  def mount(%{"token" => token}, _session, socket) do
    socket =
      case Platform.update_developer_email(socket.assigns.current_developer.developer, token) do
        {:ok, _developer} ->
          put_flash(socket, :info, "Email changed successfully.")

        {:error, _} ->
          put_flash(socket, :error, "Email change link is invalid or it has expired.")
      end

    {:ok, push_navigate(socket, to: ~p"/developers/settings")}
  end

  def mount(_params, _session, socket) do
    developer = socket.assigns.current_developer.developer
    email_changeset = Platform.change_developer_email(developer, %{}, validate_unique: false)
    password_changeset = Platform.change_developer_password(developer, %{}, hash_password: false)

    socket =
      socket
      |> assign(:current_email, developer.email)
      |> assign(:email_form, to_form(email_changeset))
      |> assign(:password_form, to_form(password_changeset))
      |> assign(:profile_form, to_form(Platform.change_developer_profile(developer)))
      |> assign(:trigger_submit, false)

    {:ok, socket}
  end

  @impl true
  def handle_event("validate_profile", %{"developer" => params}, socket) do
    changeset =
      socket.assigns.current_developer.developer
      |> Platform.change_developer_profile(params)
      |> Map.put(:action, :validate)

    {:noreply, assign(socket, :profile_form, to_form(changeset))}
  end

  def handle_event("update_profile", %{"developer" => params}, socket) do
    case Platform.update_developer_profile(socket.assigns.current_developer.developer, params) do
      {:ok, developer} ->
        scope = %{socket.assigns.current_developer | developer: developer}

        {:noreply,
         socket
         |> assign(:current_developer, scope)
         |> assign(:profile_form, to_form(Platform.change_developer_profile(developer)))
         |> put_flash(:info, "屋号・氏名を更新しました")}

      {:error, changeset} ->
        {:noreply, assign(socket, :profile_form, to_form(Map.put(changeset, :action, :insert)))}
    end
  end

  def handle_event("validate_email", params, socket) do
    %{"developer" => developer_params} = params

    email_form =
      socket.assigns.current_developer.developer
      |> Platform.change_developer_email(developer_params, validate_unique: false)
      |> Map.put(:action, :validate)
      |> to_form()

    {:noreply, assign(socket, email_form: email_form)}
  end

  def handle_event("update_email", params, socket) do
    %{"developer" => developer_params} = params
    developer = socket.assigns.current_developer.developer
    true = Platform.sudo_mode?(developer)

    case Platform.change_developer_email(developer, developer_params) do
      %{valid?: true} = changeset ->
        Platform.deliver_developer_update_email_instructions(
          Ecto.Changeset.apply_action!(changeset, :insert),
          developer.email,
          &url(~p"/developers/settings/confirm-email/#{&1}")
        )

        info = "A link to confirm your email change has been sent to the new address."
        {:noreply, socket |> put_flash(:info, info)}

      changeset ->
        {:noreply, assign(socket, :email_form, to_form(changeset, action: :insert))}
    end
  end

  def handle_event("validate_password", params, socket) do
    %{"developer" => developer_params} = params

    password_form =
      socket.assigns.current_developer.developer
      |> Platform.change_developer_password(developer_params, hash_password: false)
      |> Map.put(:action, :validate)
      |> to_form()

    {:noreply, assign(socket, password_form: password_form)}
  end

  def handle_event("update_password", params, socket) do
    %{"developer" => developer_params} = params
    developer = socket.assigns.current_developer.developer
    true = Platform.sudo_mode?(developer)

    case Platform.change_developer_password(developer, developer_params) do
      %{valid?: true} = changeset ->
        {:noreply, assign(socket, trigger_submit: true, password_form: to_form(changeset))}

      changeset ->
        {:noreply, assign(socket, password_form: to_form(changeset, action: :insert))}
    end
  end
end
