defmodule AntPressWeb.UserLive.Login do
          ...
          <.header>
            ...
            <:subtitle>
              <%= if @current_user do %>
                重要な操作を行うため、再認証が必要です。
              <% else %>
                アカウントは制作者が発行します。お持ちでない場合はお問い合わせください。
              <% end %>
            </:subtitle>
          </.header>
        </div>
  ...
  def mount(_params, _session, socket) do
    email =
      Phoenix.Flash.get(socket.assigns.flash, :email) ||
        get_in(socket.assigns, [:current_user, Access.key(:user), Access.key(:email)])
