...
## 5. TypeScript の型を OpenAPI から生成する

**推奨する。** 理由は 1 つに尽きる。**API を変更したとき、どの HP が壊れるかを
ビルド時に検出できる。**

```
Phoenix（open_api_spex）
    ↓  /api/openapi
OpenAPI 3 仕様
    ↓  openapi-typescript
antpress-api.d.ts
    ↓
Astro / React が型付きで API を叩く
```
...
| 状況 | 型なし | 型あり |
| --- | --- | --- |
| `article.titel` とタイポ | **`undefined` が表示される**（気づかない） | **ビルドエラー** |
| API のフィールド名を変更 | HP が静かに空白を表示 | **HP のビルドが失敗し即座に分かる** |
| レスポンス構造を調べる | ドキュメントを見る | エディタが補完する |

**2 番目が最も重要。** antpress は複数クライアントの HP に配信するため、
型がなければ「記事が出てこない」とクライアントから連絡が来て初めて気づくことになる。

### 注意点

- **型生成を CI に組み込む。** 手動だと仕様変更時に型が古くなり、
  「型はあるのに間違っている」という最悪の状態になる
- ⚠️ **型は 1.1 のルール（React island 内で fetch しない）を守れない。**
  型は「何が返るか」を保証するが「どこで呼ぶか」は保証しない。あのルールは別途必要
- ページ数の少ない小規模サイトでは、手間に対する効果は小さい
