...
- `storage_path` は `clients/{client_id}/{uuid}.{ext}` の形にしてテナント分離する
  - **アップロード時のファイル名は保存パスに使わない。** 日本語ファイル名や
    ディレクトリトラバーサルを構造的に排除できる
  - 拡張子は `content_type` から決める
- `storage_path` に **UNIQUE 制約**を張る。2 レコードが同じパスを指すと、
  片方を削除したときにもう片方が壊れる
- `content_type` は `Ecto.Enum` ではなく文字列 ＋ **DB の CHECK 制約**で絞る
  （`image/jpeg` / `image/png` / `image/gif` / `image/webp`）。
  **SVG は含めない**（スクリプトを埋め込めるため → `DECISIONS.md` 3.3）
- ⚠️ `inserted_at` / `updated_at` は **`utc_datetime_usec`**（マイクロ秒）。
  他のテーブルは秒精度だが、画像は 10 件まとめて上げられるので
  **同一秒に複数行入るのが普通**。秒精度だと一覧の並び順が同着になり、
  第 2 キー（ランダムな UUID）で決まってしまう

#### コンテキストは `AntPress.Media`。`Blog` には入れない

**1.5「スキーマ 1 つのコンテキストは切らない」の例外にあたる。** 理由は 2 つ。

1. **テーブル名が `images` で、`blog_` 接頭辞を持たない。** 画像はブログ専有では
   なくクライアントの素材置き場。`Blog` に入れると、将来 EC などで同じ画像を
   使うときに移動が必要になる
2. **責務がコンテンツのモデリングではない。** オブジェクトストレージへの
   書き込み・削除、MIME とサイズの検証、ヘッダからの縦横サイズ取得。
   これを `Blog` に混ぜると記事のドメインロジックにファイル I/O が同居する

ファイル本体の保存先は `AntPress.Storage`（behaviour）が抽象化する。
本番は `Supabase`、開発とテストは `Local`。**インフラなので
`lib/antpress/` 直下**に `Mailer` / `Vault` と並べて置く。

#### ⚠️ DB とストレージのどちらかが失敗したとき

別系統なので片方だけ成功しうる。**残す状態を意図的に選んでいる。**
...
どちらも「**参照先が無いレコードを作らない**」側に倒している。
孤児オブジェクトは容量を無駄にするだけだが、実体の無いレコードは
HP 上で画像が壊れて見える。問い合わせを DB 保存優先にしているのと同じ考え方。

削除でストレージが失敗しても**エラーは返さない**（ログに残して先へ進む）。
レコードは既に消えているので、エラーを返してもユーザーに再試行の手段がない。
...
### 5.4 `body_html` はキャッシュなので陳腐化する

**これは事象の記録であり、機能の予定ではない。**

`body_html` はレンダリング結果のキャッシュ（→ 3.8）。そのため
**Markdown のレンダリング設定を変えたり、レンダリングのバグを直しても、
既存記事の `body_html` は古いまま**になる。

- 新規保存・更新した記事だけが新しい結果になる
- 配信 API は `body_html` を返すので、古い記事は古い HTML のまま配信される

実際にレンダラを変更する必要が生じた時点で、一括再生成の手段を用意する。
**先回りして作らない**（機能を増やさない方針 → `DECISIONS.md` 1.2）。
